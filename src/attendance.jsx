    ...
    const handleLogin = async (e) => {
        ...
        try {
            const response = await axios.post(`${api}/api/Hack/auth/login`, {
                identifier: selectedSector,
                password: password
            });

            if (response.data.token) {
                sessionStorage.setItem("sectorToken", response.data.token);
                sessionStorage.setItem("sector", selectedSector);
                setError("");
                setIsAuthenticated(true);
            }
        } catch (err) {
            setError(err.response?.data?.error || "Authentication failed. Invalid passkey.");
        }
    };

    const handleLogout = () => {
        sessionStorage.removeItem("sectorToken");
