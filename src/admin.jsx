...
function Admin() {
    ...
    const [targetedTeamId, setTargetedTeamId] = useState("");
    const [isSendingTargetedMessage, setIsSendingTargetedMessage] = useState(false);
    const [targetedMessageError, setTargetedMessageError] = useState("");
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            setLoginError(error.response?.data?.error || "Incorrect Secret Jutsu. Access Denied.");
            setPasswordInput("");
        }
    };
    ...
    const handleSendTargetedMessage = () => {
        if (!targetedMessageText.trim()) { setTargetedMessageError("Message cannot be empty."); return; }
        if (!targetedTeamId) { setTargetedMessageError("Please select a team."); return; }
        setIsSendingTargetedMessage(true);
        setTargetedMessageError("");
        socket.emit('admin:sendTargetedMessage', { teamId: targetedTeamId, message: targetedMessageText.trim() });
        setNotification({ message: 'Targeted Message Sent!', type: 'success' });
        setTimeout(() => { setIsSendingTargetedMessage(false); setTargetedMessageText(""); }, 1000);
    };
                        ...
                        <>
                            ...
                            {activeView === 'broadcast' && (
                                <div className="space-y-6">
                                    ...
                                    {/* NEW: Targeted Team Message */}
                                    <div className="p-6 rounded-2xl" style={{ background: 'rgba(56,189,248,0.06)', border: '1px solid rgba(56,189,248,0.2)' }}>
                                        <h3 className="text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">🎯 Send Targeted Message</h3>
                                        <div className="space-y-4">
                                            <select
                                                value={targetedTeamId}
                                                onChange={(e) => setTargetedTeamId(e.target.value)}
                                                className="w-full p-4 rounded-xl text-white focus:outline-none transition-all"
                                                style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(56,189,248,0.2)' }}
                                                disabled={isSendingTargetedMessage}
                                            >
                                                <option value="" className="bg-gray-900 text-gray-500">-- Select a Team --</option>
                                                {allTeams.map(t => (
                                                    <option key={t._id} value={t._id} className="bg-gray-800 text-white">
                                                        {t.teamname} {t.verified ? '(Verified)' : ''}
                                                    </option>
                                                ))}
                                            </select>
                                            <textarea
                                                ...
                                                placeholder="e.g., Your payment is pending, please resolve..."
                                                className="w-full h-24 p-4 rounded-xl text-white placeholder-gray-600 focus:outline-none resize-none transition-all"
                                                style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(56,189,248,0.2)' }}
                                                disabled={isSendingTargetedMessage}
                                            />
                                        </div>
                                        {targetedMessageError && <p className="text-red-400 text-sm mt-2">{targetedMessageError}</p>}
                                        <button
                                            onClick={handleSendTargetedMessage}
                                            disabled={isSendingTargetedMessage || !targetedMessageText.trim() || !targetedTeamId}
                                            className="mt-4 w-full font-bold py-3 rounded-xl text-white disabled:opacity-40 transition-all hover:scale-[1.01]"
                                            style={{ background: 'linear-gradient(135deg, #0284c7, #38bdf8)' }}>
                                            {isSendingTargetedMessage ? 'Sending Message...' : '✉️ Send Direct Message'}
                                        </button>
                                    </div>
